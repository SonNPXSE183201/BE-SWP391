...
* **Cập nhật Hồ sơ Người dùng (User Profile)**: Triển khai API `/api/v1/profile` (PUT) tự động xác định Role để cập nhật các field (Admin/Mangaka cập nhật Tên, Bút danh; Assistant cập nhật Tên, Kỹ năng, Tags, URL) mà không sửa đổi các controller cũ. Đã tích hợp testcase.
