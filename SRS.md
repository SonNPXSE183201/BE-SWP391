...
| **F4.2** | Cast Vote | Publishing | Bỏ phiếu Đồng ý/Từ chối duyệt truyện, nhập số tiền đề xuất cấp ngân sách sản xuất và ghi nhận xét. | Mỗi thành viên hội đồng chỉ được bỏ phiếu đúng 1 lần cho mỗi bộ truyện. |
...
| **Withdrawal** | Mangaka / Assistant | Rút tiền khả dụng từ ví về tài khoản ngân hàng cá nhân. | **Thủ công (Manual)**: Khởi tạo yêu cầu rút từ số dư `WithdrawableBalance`, chuyển khoản qua kết nối giả lập VNPay Sandbox API. |
